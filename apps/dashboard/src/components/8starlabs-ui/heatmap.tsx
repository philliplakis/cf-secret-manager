...
function daysOfTheWeekIndicator({ daysOfTheWeekOption, fontSize }: DaysOfTheWeekIndicatorProps) {
...
export default function Heatmap(props: HeatmapProps) {
  ...
  const valueByDate = new Map<string, number>(data.map(({ date, value }) => [date, value]));
    ...
    >
      ...
      <TooltipProvider>
        {weeks.map((week, weekIdx) =>
          week.map((day, dayIdx) => {
            if (!day) {
              return <div key={dayIdx} style={{ gridColumn: weekIdx + 2, gridRow: dayIdx + 2 }} />;
            }
